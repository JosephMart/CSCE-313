...
int main(int argc, char **argv) {
    ...
    // Initialize values for reading the parameters
    int c = 0;
    extern char *optarg;

    // Default block and mem size
    unsigned int block_size = 128;
    unsigned int mem_size = 512000;

    // Read in the parameters
    while ((c = getopt(argc, argv, "b:s:")) != -1) {
        switch (c) {
            case 's':
                mem_size = (unsigned int) strtoul(optarg, NULL, 10);
                break;
            case 'b':
                block_size = (unsigned int) strtoul(optarg, NULL, 10);
                break;
            default:
                break;
        }
